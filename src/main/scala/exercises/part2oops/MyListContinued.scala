...
import exercises.part2oops.MyListContinued.MyTransformer
...
object MyListContinued extends App {
  ...
  object myEmptyList extends myList[Nothing] {
    ...
    override def map[B](transformer: MyTransformer[Nothing, B]): myList[B] = myEmptyList
    def flatMap[B](transformer: MyTransformer[Nothing, myList[B]]): myList[B] = myEmptyList
    override def filter(predicate: MyPredicate[Nothing]): myList[Nothing] = myEmptyList
  ...
  class myCons[+A](h: A, t: myList[A]) extends myList[A] {
    ...
    override def filter(predicate: MyPredicate[A]): myList[A] = {
      if(predicate.test(this.h)) new myCons[A](this.h, t.filter(predicate))
    ...
    override def map[B](transformer: MyTransformer[A, B]): myList[B] = {
      new myCons[B](transformer.transform(h), t.map(transformer))
    }
    /*
    [1,2] ++ [3,4,5]
...
object runner extends App {
  ...
  println(listOfInt)
  println(listOfInt.map(new MyListContinued.MyTransformer[Int, Int] {
    override def transform(data: Int): Int = data * 2
  }))
  println(listOfInt.filter(new MyListContinued.MyPredicate[Int] {
    override def test(data: Int): Boolean = data % 2 == 1
  }))
  println(listOfInt ++ listOfInt2)
  println(listOfInt.flatMap(new MyTransformer[Int, MyListContinued.myList[Int]] {
    override def transform(data: Int): MyListContinued.myList[Int] = new MyListContinued.myCons[Int](data, new MyListContinued.myCons[Int](data + 1, MyListContinued.myEmptyList))
  }))
}